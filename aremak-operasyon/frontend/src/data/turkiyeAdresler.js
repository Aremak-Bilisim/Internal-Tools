...
export const IL_ILCE = {
  ...
  'Ordu': ['Altınordu','Aybastı','Çamaş','Çatalpınar','Çaybaşı','Fatsa','Gölköy','Gülyalı','Gürgentepe','İkizce','Kabadüz','Kabataş','Korgan','Kumru','Mesudiye','Perşembe','Ulubey','Ünye'],
  ...
  'Trabzon': ['Akçaabat','Araklı','Arsin','Beşikdüzü','Çarşıbaşı','Çaykara','Dernekpazarı','Düzköy','Hayrat','Köprübaşı','Maçka','Of','Ortahisar','Pelitli','Shebinci','Sürmene','Şalpazarı','Tonya','Vakfıkebir','Yomra'],
...
// Normalize: Türkçe karakterleri dönüştür, küçük harfe çevir
...
// Düz arama için hazırlanmış lookup tablosu
// { normalizedName: { type: 'il'|'ilce', il: string, ilce?: string } }
const _lookup = {}

for (const [il, ilceler] of Object.entries(IL_ILCE)) {
  const normIl = normalize(il)
  if (!_lookup[normIl]) _lookup[normIl] = { type: 'il', il }
  for (const ilce of ilceler) {
    const normIlce = normalize(ilce)
    if (!_lookup[normIlce]) _lookup[normIlce] = { type: 'ilce', il, ilce }
  }
}
